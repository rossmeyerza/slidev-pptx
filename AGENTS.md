...
- PPTX export has two formats. (1) **Flattened** (`pptx`): pixel-perfect Playwright screenshots via pptxgenjs — the default, lossless-fidelity artifact; keep it. (2) **Native/editable** (`pptx-native`, in progress as of 2026-07-13): HTML → IR → real pptxgenjs shapes, where the IR is built from **browser-computed geometry** (Playwright walks the rendered DOM reading getBoundingClientRect + getComputedStyle), with a per-element raster fallback for anything that won't map cleanly (gradients, shadows, transforms, SVG). Native is intentionally lossy but produces editable text/shapes. This reverses the earlier "editable PPTX is a non-goal" decision (Ross, 2026-07-13). DOM-to-shapes is now an approved path via the browser-geometry approach; do NOT hand-parse HTML/CSS to build the IR — let the browser compute layout.
