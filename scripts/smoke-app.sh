...
[[ -f "$WORK_DIR/data/decks/$custom_deck_id/runtime.js" ]] || fail "custom runtime deck should copy runtime.js"
[[ -f "$WORK_DIR/data/decks/$custom_deck_id/theme.css" ]] || fail "custom runtime deck should copy theme.css"
[[ -f "$WORK_DIR/data/decks/$custom_deck_id/deck.json" ]] || fail "custom runtime deck should copy deck.json"
grep -q 'Custom Runtime Smoke' "$WORK_DIR/data/decks/$custom_deck_id/deck.json" || fail "deck.json title should be set from the deck title"
renamed_custom_payload="$(curl -fsS -b "$COOKIE_JAR" -X PUT "http://127.0.0.1:$PORT/api/decks/$custom_deck_id" -H 'content-type: application/json' --data '{"title":"Renamed Runtime Smoke"}')"
printf '%s' "$renamed_custom_payload" | grep -q '"title":"Renamed Runtime Smoke"' || fail "rename should update the deck payload"
grep -q '"title": "Renamed Runtime Smoke"' "$WORK_DIR/data/decks/$custom_deck_id/deck.json" || fail "rename should update deck.json title"
files_payload="$(curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/api/decks/$custom_deck_id/files")"
printf '%s' "$files_payload" | grep -q '"path":"deck.json"' || fail "files API should list deck.json"
printf '%s' "$files_payload" | grep -q '"path":"slides/01-title.html"' || fail "files API should list manifest slides"
curl -fsS -b "$COOKIE_JAR" -X PUT "http://127.0.0.1:$PORT/api/decks/$custom_deck_id/files/content" -H 'content-type: application/json' --data '{"path":"slides/01-title.html","content":"<section class=\"slide\" data-title=\"Manual smoke\"><h1>Manual file smoke</h1></section>"}' | grep -q '"ok":true' || fail "files API should save a slide"
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/runtime/$custom_deck_id/slides/01-title.html" | grep -q 'Manual file smoke' || fail "manual file edit should be served by the runtime"
invalid_manifest_status="$(curl -sS -o "$WORK_DIR/invalid-manifest.json" -w '%{http_code}' -b "$COOKIE_JAR" -X PUT "http://127.0.0.1:$PORT/api/decks/$custom_deck_id/files/content" -H 'content-type: application/json' --data '{"path":"deck.json","content":"not json"}')"
[[ "$invalid_manifest_status" == "400" ]] || fail "invalid deck.json should return 400, got HTTP $invalid_manifest_status"
duplicate_payload="$(curl -fsS -b "$COOKIE_JAR" -X POST "http://127.0.0.1:$PORT/api/decks/$custom_deck_id/duplicate" -H 'content-type: application/json' --data '{}')"
duplicate_id="$(printf '%s' "$duplicate_payload" | json_string id)"
[[ "$duplicate_id" != "$custom_deck_id" ]] || fail "duplicate should return a new id"
[[ -f "$WORK_DIR/data/decks/$duplicate_id/slides/01-title.html" ]] || fail "duplicate should copy slide files"
[[ ! -e "$WORK_DIR/data/decks/$duplicate_id/.snapshots" ]] || fail "duplicate should exclude snapshots"
curl -fsS -b "$COOKIE_JAR" -X DELETE "http://127.0.0.1:$PORT/api/decks/$duplicate_id" | grep -q '"ok":true' || fail "duplicate delete should return ok"
deleted_status="$(curl -sS -o /dev/null -w '%{http_code}' -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/api/decks/$duplicate_id")"
[[ "$deleted_status" == "404" ]] || fail "deleted deck should return 404, got HTTP $deleted_status"
template_payload="$(curl -fsS -b "$COOKIE_JAR" -X POST "http://127.0.0.1:$PORT/api/decks/$custom_deck_id/template" -H 'content-type: application/json' --data '{"name":"Smoke Saved Template","description":"Created by smoke"}')"
printf '%s' "$template_payload" | grep -q '"key":"smoke-saved-template"' || fail "save-as-template should return the scaffold"
[[ -f "$WORK_DIR/data/templates/smoke-saved-template/deck.json" ]] || fail "save-as-template should create the template directory"
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/api/scaffolds" | grep -q '"key":"smoke-saved-template"' || fail "saved template should appear in scaffold list"
templated_deck_payload="$(curl -fsS -b "$COOKIE_JAR" -X POST "http://127.0.0.1:$PORT/api/decks" -H 'content-type: application/json' --data '{"title":"From Saved Template","scaffold":"smoke-saved-template"}')"
printf '%s' "$templated_deck_payload" | grep -q '"scaffoldKey":"smoke-saved-template"' || fail "deck should be creatable from saved template"
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/runtime/$custom_deck_id/" | grep -q 'deck-stage' || fail "custom runtime route should serve the runtime shell"
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/runtime/$custom_deck_id/theme.css" | grep -q -- '--accent' || fail "custom runtime route should serve theme.css"
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/runtime/$custom_deck_id/deck.json" | grep -q '"slides"' || fail "custom runtime route should serve deck.json"
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/runtime/$custom_deck_id/slides/01-title.html" | grep -q 'class="slide' || fail "custom runtime route should serve slide fragments"
...
branded_deck_payload="$(curl -fsS -b "$COOKIE_JAR" -X POST "http://127.0.0.1:$PORT/api/decks" -H 'content-type: application/json' --data '{"title":"Branded Runtime Smoke","scaffold":"commercial-html"}')"
branded_deck_id="$(printf '%s' "$branded_deck_payload" | json_string id)"
...
share_payload="$(curl -fsS -b "$COOKIE_JAR" -X POST "http://127.0.0.1:$PORT/api/decks/$deck_id/shares" -H 'content-type: application/json' --data '{"name":"Smoke Client","email":"client@example.com","permission":"view","expiresInDays":7}')"
...
printf '%s' "$share_payload" | grep -q '"expiresAt"' || fail "expiring share should return expiresAt"
...
curl -fsS -b "$COOKIE_JAR" "http://127.0.0.1:$PORT/api/decks/$deck_id" | grep -q '"viewCount":2' || fail "two public share payload visits should increment viewCount twice"
...
edit_share_id="$(printf '%s' "$edit_share_payload" | node -e "const fs=require('fs'); const input=JSON.parse(fs.readFileSync(0,'utf8')); process.stdout.write(input.share.id)")"
